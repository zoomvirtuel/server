...
const getProductoPrecioAndCantidad = async () => {
  try {
    ...
    const productosConInfo = productos.map((producto) => {
      ...
      const cantidadVentas = producto.venta.reduce((total, venta) => total + venta.cantidad, 0);

      // Calcula la cantidad de compras
      const cantidadCompras = producto.compra.reduce((total, compra) => total + compra.cantidad, 0);
      ...
      return {
        ...
        precioVentaDiferido: ultimaCompra ? ultimaCompra.precioVentaDiferido : null,
